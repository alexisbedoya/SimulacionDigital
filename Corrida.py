...
def operacion(NSA):
  ...
  for i in range(len(corridas)):
    if i < len(corridas)-1:
      if (corridas[i-1] !=0 ) & (corridas[i] ==0 ) & (corridas[i+1] != 0):
        count0 =count0 + 1
      elif (corridas[i-1] !=1 ) & (corridas[i] ==1 ) & (corridas[i+1] != 1):
          count1 =count1 +1
    else:
      if (corridas[i-1] !=0 ) & (corridas[i] ==0 ):
        count0 =count0 + 1
      elif (corridas[i-1] !=1 ) & (corridas[i] ==1 ):
        count1 =count1 +1
  print("numeros de ceros: ", count0, "Numeros de unos: ", count1)
